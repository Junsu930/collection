...
public class StudentService {
	...
	// 학생 정보를 저장할 List를 생성 (객체 배열 Upgrade 버젼)
	
	// java.util.List 인터페이스 : List에 반드시 필요한 필수 기능 모아둔 인터페이스
	// *인터페이스는 객체 생성 X , 부모 참조 변수 O 
	
	// java.util.ArrayList : 배열 형태 List (가장 대표적인 List 자식 클래스) 
	
	// ArrayList() 기본 생성자 : 기본 크기 10짜리 리스트 생성
	//						리스트 크기가 늘었다 줄었다 하기 때문에 큰 의가 없다.
	...
	public void ex() {
		...
		// (반환형)
		// List.get(index i) : 인덱스 i번째 인덱스에 있는 객체(Object)를 반환
	...
	public void displayMenu() {
		...
		do {
			...
			try {
				...
				menuNum = -1; // 첫 반복 시 잘못 입력하는 경우 menuNum이 0을 갖고 있어서 종료됨
			}
		}while(menuNum != 0);
	...
	public void selectAll() {
		...
		// List가 비어있을 시 "학생 정보가 없습니다." 출력
		
		// List가 비어있는지 확인하는 방법
		// boolean List.isEmpty() : 비어있으면 true
		//if(studentList.size()==0) {
		if(studentList.isEmpty()) { // studentList 가 비어있으면 true 반환
			System.out.println("학생 정보가 없습니다.");
			return; // 반환형 없는 return 구문은 현재 메소드를 종료하고 호출한 곳으로 돌아감 
		...
		*/ 
		...
		//[작성법]
		// for( 컬렉션 or 배열에서 꺼낸 한 개의 요소를 저장할 변수 : 컬렉션명 또는 배열명){}
		int index = 0;
		for( Student std : studentList) {
			// std에는 for문 반복 시마다 0, 1, 2 , .. 인덱스 요소들을 한 번씩 저장됨.
	...
	public String updateStudent() throws InputMismatchException {
		...
		if( studentList.isEmpty()) {
			...
			// 2) 입력된 숫자가 0보다 작은가? (음수X)
		}else if ( index<0 ) {
			return "음수는 입력할 수 없습니다.";
			// + 문자열을 입력한 경우 => throws로 예외처리 
			// 3) 입력된 숫자가 studentList 범위 내의 인덱스 번호인가?
			...
			// 입력 받은 index번 째에 새로운 학생 정보를 세팅
			// 이 때 index번 째에 있던 기존 학생 정보 반환
	...
	public String removeStudent() throws InputMismatchException {
		...
		if( studentList.isEmpty()) {
			...
			char ch = sc.next().toUpperCase().charAt(0);
					// String 형 -> 대문자 String -> 0번째 인덱스 문자
					// String.toUpperCase() : 문자열을 대문자로 변경
					// String.toLowerCase() : 문자열을 대문자로 변경
			
			if(ch == 'Y') {
				
				Student temp = studentList.remove(index);
				return "\n" + temp.getName() + "의 정보가 제거되었습니다.";
	...
	public void searchName2() {
		...
		System.out.print("이름에 포함될 문자열 검색 : ");
		String input = sc.next();
